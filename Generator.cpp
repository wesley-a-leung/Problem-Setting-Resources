...
std::mt19937 rng; std::mt19937_64 rng64;

const string CHECKER = "";

const int GLOBAL_SAMPLES = 0, BATCHES = 0;

const int BATCH_POINTS[BATCHES] = {};
const int CASES_PER_BATCH[BATCHES] = {};
const int SAMPLES_PER_BATCH[BATCHES] = {};

const ll SEED = 0x3f3f3f3f;

char buf[1000];

void genCase(int batchNumber, int caseNumber) {
  setDelimiter("");
  debugln("  - {in: data/", batchNumber, ".", caseNumber, ".in, out: data/", batchNumber, ".", caseNumber, ".out}");
  fill(buf, buf + 1000, 0);
  sprintf(buf, "data/%d.%d.in", batchNumber, caseNumber);
  setOutput(buf);
  setDelimiter(" ");
  // Input
  fill(buf, buf + 1000, 0);
  sprintf(buf, "data/%d.%d.out", batchNumber, caseNumber);
  setOutput(buf);
  setDelimiter(" ");
  // Output
}

void genBatch(int batchNumber) {
  setDelimiter("");
  debugln("- batched:");
  for (int sampleNumber = 1; sampleNumber <= SAMPLES_PER_BATCH[batchNumber - 1]; sampleNumber++) debugln("  - {in: data/", batchNumber, ".sample.", sampleNumber, ".in, out: data/", batchNumber, ".sample.", sampleNumber, ".out}");
  for (int caseNumber = 1; caseNumber <= CASES_PER_BATCH[batchNumber - 1]; caseNumber++) genCase(batchNumber, caseNumber);
  setDelimiter("");
  debugln("  points: ", BATCH_POINTS[batchNumber - 1]);
}

int main(int argc, char* argv[]) {
  setDelimiter("");
  setError("init.yml");
  debugln("archive: data.zip");
  debugln("checker: ", CHECKER);
  debugln("test_cases:");
  rng = mt19937(SEED);
  rng64 = mt19937_64(SEED);
  for (int sampleNumber = 1; sampleNumber <= GLOBAL_SAMPLES; sampleNumber++) debugln("- {in: data/0.sample.", sampleNumber, ".in, out: data/0.sample.", sampleNumber, ".out, points: 0}");
  for (int batchNumber = 1; batchNumber <= BATCHES; batchNumber++) genBatch(batchNumber);
  return 0;
}
